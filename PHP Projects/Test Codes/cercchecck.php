...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if(empty($ccard) || empty($mmonth) || empty($yyear) || empty($ccvc)){
        ...
        $log_err="Please correcctly fill the card number"
    }else if($yyear >= $year){
        if($mmonth >= $month){
            ...
            if ($err) {
                ...
                if ($err) {
                    ...
                    if ($err) {
                        ...
                        $amount=100000;
                        $brand=$dfinal->data->attributes->payments->source->brand;
                        $fourdigi=$dfinal->data->attributes->payments->source->last4;

                        $sql="INSERT INTO paymentmongo VALUES (?,?,?,'paid','card',?,?)";
                        if($stmt=$conn->prepare($sql)){
                            $stmt->bind_param("sssss", $parampay, $paramres, $paramamount, $parambrand, $paramlast);
                            $parampay=$payid;
                            $paramres=$resid;
                            $paramamount=$amount;
                            $parambrand=$brand;
